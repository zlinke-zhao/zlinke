...
export const tools: Tool[] = [
  ...
  },
  {
    ...
    price: '注册赠 $3 额度、无需信用卡；无订阅费与席位费，预付额度按两条计量线扣除——模型 token 按各厂商公开价计、算力按机器唤醒秒数计（4GB 默认机约 $0.071/唤醒小时）；挂起与停放的 Agent 免费；可绑定自有 Claude Pro/Max 或 ChatGPT 订阅使模型侧零计费',
    features: [
      'Harness 无关：Claude Code / Codex / Hermes / OpenClaw 四选一，harness 本身免费，事后可更换且不丢历史',
      '模型无关：Claude Fable 5/Opus 5/Sonnet 4.6、GPT-5.6 Sol/Terra/Luna、Gemini 3.5 Flash、DeepSeek V4 Pro/Flash、GLM-5.2、Kimi K3 可随时切换',
      '全渠道可达：WhatsApp / iMessage / Telegram / Slack / Web / A2A 协议 / CLI 共用同一 Agent 的历史、工具与状态',
      'sky clone：一句提示词把本机已配好的 Claude Code / Codex / OpenClaw 连同指令文件与 MCP Server 克隆上云，无需重建',
      '持久化运行时：托管 worker 配快照、备份与故障自动恢复，官方称连续历史长度无上限',
      '停放机制：闲置自动挂起、停放期零费用，可维持一支 Agent 舰队而不为静默付费',
      '自带订阅：绑定 Claude Pro/Max 或 ChatGPT 计划后，符合条件的 Agent 模型侧零计费',
      '开箱即用能力：网页抓取(Exa)、浏览器操作(TinyFish)、SERP 搜索(DataForSEO)、生图、去背景、生视频(Seedance)、图生视频(MiniMax)、语音转写(Fish Audio) 及 Gmail/Notion/Slack 连接器，另接 2000+ 连接器',
      '开发者优先：UI 上的每个动作都有对应 CLI 命令与 REST API，可接入 CI/CD 或嵌入自有产品',
      '支持 Zero Data Retention（ZDR），面向有数据留存合规要求的团队',
      '2026-08-03 登陆 Product Hunt 当日第一（469 赞 / 60 评论），由 Product Hunt CEO Rajiv Ayyangar 提名',
      '自进化（Self-evolving）能力官网标注 coming soon，尚未上线'],
  },
  {
    id: 'browseros-neo',
    name: 'BrowserOS neo',
